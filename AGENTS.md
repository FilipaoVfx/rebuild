# Urban Recovery Intelligence — agent guide

Read `docs/plan/antes-de-empezar.md` before changing product scope, data sources,
scoring, or published outputs. Treat the PRD, SRS, ARD, and ADRs as the source of
...
(cd apps/viewer && npm run typecheck && npm run build)
.venv/bin/python scripts/checks/browser_check.py
.venv/bin/python scripts/checks/signal_check.py
```

Do not add synthetic urban data. Versioned sources live under `db/seed/`, and
the `rebuild_` PostgreSQL schema prefixes are part of the architecture contract.
