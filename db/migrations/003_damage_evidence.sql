...
CREATE TABLE rebuild_core.damage_evidence (
    evidence_id      bigserial PRIMARY KEY,

    -- fuentes.md §7.9 — un agregador CC BY 4.0 no "lava" la licencia de lo
    -- que agrega. Por eso son dos columnas y no una.
    source           text NOT NULL REFERENCES rebuild_core.source_register(source_id),
    original_source  text NOT NULL REFERENCES rebuild_core.source_register(source_id),
    ...
    damage_class     rebuild_core.damage_class NOT NULL,
    raw_damage_label text NOT NULL,
    building_type    text,
    method           rebuild_core.evidence_method NOT NULL,
...
CREATE INDEX damage_evidence_geom_idx ON rebuild_core.damage_evidence USING GIST (geometry);
CREATE INDEX damage_evidence_source_idx ON rebuild_core.damage_evidence (original_source);

COMMENT ON TABLE rebuild_core.damage_evidence IS
...
CREATE TABLE rebuild_core.site (
    ...
    state            rebuild_core.site_state NOT NULL DEFAULT 'INGESTED',
...
CREATE INDEX site_geom_idx ON rebuild_core.site USING GIST (geometry);
CREATE INDEX site_centroid_idx ON rebuild_core.site USING GIST (centroid);
CREATE INDEX site_state_idx ON rebuild_core.site (state);
...
CREATE TABLE rebuild_core.site_evidence (
    site_id      text   NOT NULL REFERENCES rebuild_core.site(site_id) ON DELETE CASCADE,
    evidence_id  bigint NOT NULL REFERENCES rebuild_core.damage_evidence(evidence_id),
    PRIMARY KEY (site_id, evidence_id)
);

-- ADR-16 — la fusion: de N observaciones a una clase y una confianza.
CREATE TABLE rebuild_core.site_damage_fusion (
    site_id            text PRIMARY KEY REFERENCES rebuild_core.site(site_id) ON DELETE CASCADE,
    damage_class       rebuild_core.damage_class NOT NULL,
