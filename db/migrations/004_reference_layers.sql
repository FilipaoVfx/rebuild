...
CREATE INDEX osm_road_geom_idx ON osm_raw.road USING GIST (geometry);

CREATE TABLE osm_raw.green_space (
    osm_id      bigint PRIMARY KEY,
...
CREATE INDEX osm_green_geom_idx ON osm_raw.green_space USING GIST (geometry);

CREATE TABLE osm_raw.facility (
    osm_id      bigint PRIMARY KEY,
...
CREATE INDEX osm_facility_geom_idx ON osm_raw.facility USING GIST (geometry);

-- osm_derived — el grafo peatonal. Deriva de OSM, luego hereda su regimen.
CREATE TABLE osm_derived.pedestrian_edge (
...
CREATE INDEX pedestrian_edge_geom_idx ON osm_derived.pedestrian_edge USING GIST (geometry);
CREATE INDEX pedestrian_edge_source_idx ON osm_derived.pedestrian_edge (source);
CREATE INDEX pedestrian_edge_target_idx ON osm_derived.pedestrian_edge (target);

-- Capas de contexto que NO derivan de OSM viven en core: distinto regimen.
CREATE TABLE core.population_cell (
...
CREATE INDEX population_cell_geom_idx ON core.population_cell USING GIST (geometry);

CREATE TABLE core.risk_zone (
...
CREATE INDEX risk_zone_geom_idx ON core.risk_zone USING GIST (geometry);

CREATE TABLE core.land_use (
