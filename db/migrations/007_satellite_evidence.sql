...
CREATE TYPE core.satellite_collection AS ENUM ('sentinel-1-grd', 'sentinel-2-l2a');
...
CREATE TYPE core.event_window AS ENUM ('PRE', 'POST');

CREATE TABLE core.satellite_scene (
    ...
    collection       core.satellite_collection NOT NULL,
    event_window     core.event_window NOT NULL,
...
CREATE INDEX satellite_scene_footprint_idx ON core.satellite_scene USING GIST (footprint);
CREATE INDEX satellite_scene_window_idx ON core.satellite_scene (collection, event_window);
...
ALTER TABLE core.satellite_scene
...
COMMENT ON TABLE core.satellite_scene IS
...
CREATE TABLE core.satellite_observation (
    ...
    event_window core.event_window NOT NULL,
    scene_pk     bigint NOT NULL REFERENCES core.satellite_scene(scene_pk),
    observation_date timestamptz NOT NULL,
    data_version bigint NOT NULL REFERENCES core.dataset_version(data_version)
);
CREATE INDEX satellite_observation_geom_idx ON core.satellite_observation USING GIST (geometry);
CREATE INDEX satellite_observation_band_idx ON core.satellite_observation (band, event_window);
...
COMMENT ON COLUMN core.satellite_observation.band IS
...
CREATE OR REPLACE FUNCTION core.reject_synthetic_satellite_version()
RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM core.dataset_version
...
CREATE TRIGGER satellite_scene_rejects_synthetic
    BEFORE INSERT OR UPDATE ON core.satellite_scene
    FOR EACH ROW EXECUTE FUNCTION core.reject_synthetic_satellite_version();

CREATE TRIGGER satellite_observation_rejects_synthetic
    BEFORE INSERT OR UPDATE ON core.satellite_observation
    FOR EACH ROW EXECUTE FUNCTION core.reject_synthetic_satellite_version();

COMMENT ON TABLE core.satellite_observation IS
